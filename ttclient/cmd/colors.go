...
func FormatRow(cols ...interface{}) []string {
	formattedRow := make([]string, len(cols))
	for i, col := range cols {

		if str, ok := col.(string); ok {
			// is string
			formattedRow[i] = Paint(WhiteText, str)
		} else {
...
func PaintErr(err error) string {
	return Paint(BrightRedBackground+BrightWhiteText, "ERROR: "+err.Error())
}
